...
summary_aux %>%
  ...
  left_join( summary_aux %>% 
               ...
               mutate( 
                 pct = `1` / (`1`+`2`),
                 se = pct * (1 - pct) / (`1`+`2`),
                 lint = round(100*(pct - 1.96*se), 4 ),
                 lsup = round(100*(pct + 1.96*se), 4 ),
                 pct = round(100*pct, 4), 
                 pct_depression = paste(pct, "(", lint, ",", lsup, ")")
               ) %>% 
               select( combined, hearing_impaired, pct_depression)
             ) %>%
  left_join( summary_aux %>% 
               filter( libras == 'yes') %>% 
               group_by(combined, hearing_impaired, diagnosed_depression) %>% 
               ...
               mutate( 
                 ...
                 lsup = round(100*(pct + 1.96*se), 4 ),
                 pct = round(100*pct, 4), 
                 pct_depression_among_libras_users = paste(pct, "(", lint, ",", lsup, ")")
               ) %>%
               select( combined, hearing_impaired, pct_depression_among_libras_users)
             ) %>% 
  ungroup() %>%
  select( -combined ) %>%
  knitr::kable()
