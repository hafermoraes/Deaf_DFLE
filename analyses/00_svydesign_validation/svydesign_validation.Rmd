...
```{get_pns_data, include=TRUE}
pns2019 <- get_pns(year=2019)
```

```{svydesign_validation_pnsibge, include=TRUE}
...
```{svydesign_validation_pnsibge_total_by_sex, include=TRUE}
...
```{svydesign_validation_pnsibge_overall_total, include=TRUE}
